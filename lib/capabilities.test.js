...
const { buildCapabilityStates, buildCommandPayload, collapseValueLists } = require('./capabilities');
...
describe('collapseValueLists', () => {
  it('turns a map of empty members into the JSON list of its keys', () => {
    const document = collapseValueLists({ doorState: { access: 'read', type: 'string', values: { CLOSED: {}, OPEN: {} } } });
    expect(document.doorState.values).to.equal('["CLOSED","OPEN"]');
  });

  it('leaves a map whose members carry overrides alone', () => {
    const capabilities = {
      program: { access: 'readwrite', type: 'string', values: { PIZZA: { targetTemperatureC: { max: 300 } } } },
    };
    expect(collapseValueLists(capabilities).program.values.PIZZA.targetTemperatureC).to.deep.equal({ max: 300 });
  });

  it('collapses the enums nested inside a program option', () => {
    const capabilities = {
      program: { values: { PIZZA: { foodProbeSupported: { access: 'readwrite', values: { NO: {}, YES: {} } } } } },
    };
    const document = collapseValueLists(capabilities);
    expect(document.program.values.PIZZA.foodProbeSupported.values).to.equal('["NO","YES"]');
  });

  it('collapses the enums inside an array of trigger rules', () => {
    const capabilities = {
      applianceState: {
        triggers: [
          { action: { executeCommand: { access: 'write', values: { START: {} } } }, condition: { operator: 'eq' } },
        ],
      },
    };
    const document = collapseValueLists(capabilities);
    expect(document.applianceState.triggers[0].action.executeCommand.values).to.equal('["START"]');
    expect(document.applianceState.triggers[0].condition).to.deep.equal({ operator: 'eq' });
  });

  it('survives a missing or malformed capability document', () => {
    expect(collapseValueLists(undefined)).to.equal(undefined);
    expect(collapseValueLists('nope')).to.equal('nope');
    expect(collapseValueLists({})).to.deep.equal({});
  });
});
