...
describe('lib/derived', () => {
  ...
  it('reports a running appliance with a finish time in milliseconds', () => {
    const result = derive(payload({ applianceState: 'RUNNING', timeToEnd: 1800 }), {}, NOW);
    expect(result.running).to.equal(true);
    expect(result.finishTime).to.equal(Date.parse('2026-01-01T12:30:00.000Z'));
  ...
  it('clears the finish time when idle or when the appliance reports -1', () => {
    ...
    expect(zero.finishTime).to.equal(null);
  });
  ...
  it('keeps the previous finishTime while the estimate only jitters', () => {
    ...
    expect(third.finishTime).to.equal(Date.parse('2026-01-01T13:00:10.000Z'));
  });
...
describe('metricsToReported', () => {
  it('turns a WebSocket metrics array into the reported shape of a poll', () => {
    ...
    expect(derive({ properties: { reported: metricsToReported(metrics) } }, {}, NOW).finishTime).to.equal(
      NOW + 120 * 1000,
    );
  });
