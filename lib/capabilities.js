...
function describeCapability(key, container, definition) {
  ...
  if (valueKeys.length > 0) {
    ...
    return {
      kind: 'plain',
      // ioBroker has no generic `level.mode`; the `level.mode.*` roles are all number
      // typed, so a writable selection stays a text state with a states map.
      common: { name: name, type: 'string', role: 'text', states: states, read: readable, write: true },
    };
  }
...
/**
 * Turn the enum maps of a capability document into plain lists.
 *
 * The cloud writes an enum as a map whose keys are the values, with the per value
 * overrides behind them - and a plain enum has none, so every member arrives as
 * `{}`. json2iob maps every object to a channel and only leaves to states, so each
 * member became an empty channel: on one oven that is 140 objects holding nothing,
 * with the whole vocabulary buried in their object ids.
 *
 * A map whose members are all empty therefore becomes the JSON list of its keys,
 * which json2iob writes as one readable state. A map whose members do carry
 * overrides - `program.values.AUGRATIN` with its temperature range - is left alone,
 * and so is a member of such a map that happens to be empty.
 *
 * Arrays are walked as well, because the `triggers` rules carry enums too. Their
 * object ids are not derived here: json2iob names array elements by a heuristic of
 * its own, so the stale channels of an older version are found by their shape
 * instead (see `removeCollapsedValueChannels`).
 *
 * @param {any} capabilities - raw capability document
 * @returns {any} - the document with every plain enum replaced by a JSON list
 */
function collapseValueLists(capabilities) {
  if (Array.isArray(capabilities)) {
    return capabilities.map((member) => collapseValueLists(member));
  }
  if (!capabilities || typeof capabilities !== 'object') {
    return capabilities;
  }
  const entries = Object.entries(capabilities);
  if (entries.length && entries.every(([, member]) => isEmptyObject(member))) {
    return JSON.stringify(entries.map(([key]) => key));
  }
  /** @type {Record<string, any>} */
  const result = {};
  for (const [key, member] of entries) {
    result[key] = collapseValueLists(member);
  }
  return result;
}
